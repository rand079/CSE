...
// Stream that has a given chance every instance per class to switch to a perturbed state/back again

public class RecurrentClassDriftStream extends AbstractOptionHandler implements InstanceStream {
    ...
    public FloatOption driftChanceOption = new FloatOption("driftChance", 'n',
            "Chance per instance to drift class", 0.0005);   
	...
	public Example<Instance> nextInstance() {
		...
		if(rng.nextDouble() < driftChance){
			...
			driftState[(int)thisClass] = !driftState[(int)thisClass];
		}
		
		//Scale instance attributes
		if(driftState[(int)thisClass]){
	...
	public void restart() {
        ...
        driftState = new boolean[numClasses];
        Arrays.fill(numNominalValues, 0);
        Arrays.fill(driftState, false);
